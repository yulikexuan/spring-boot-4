...
Some modules have their own `CLAUDE.md` (e.g. [`rest-mvc/CLAUDE.md`](rest-mvc/CLAUDE.md)) — consult them when working inside that module.
...
| `donut-shop`   | Jackson 3 messaging demo + Jackson 2 backward-compatibility toggle                |
...
1. **Hand-rolled** (in `di` module, `spring/boot/di/domain/model/jdk25/hkj/optics/` package): `Lens`, `Prism`, and `Traversal` are plain Java records with `andThen` composition. Use these to understand the mechanics.
...
```xml
<argLine>-Xshare:off -javaagent:${settings.localRepository}/org/mockito/mockito-core/${mockito.final.version}/mockito-core-${mockito.final.version}.jar</argLine>
```
